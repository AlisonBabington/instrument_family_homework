...
InstrumentFamilies.prototype.bindEvents = function () {
  ...
  PubSub.subscribe('SelectView:instrument-change' , (event) => {
    const selectFamily = event.detail
    this.publishInstrumentFamily(selectFamily);
  });
};

InstrumentFamilies.prototype.publishInstrumentFamily = function (selectFamily) {
  const foundFamily = this.instrumentFamilies[selectFamily];
  PubSub.publish('Instrument-families:selected-family', foundFamily);
};
